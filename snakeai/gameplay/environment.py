...
class Environment(object):
    ...
    def timestep(self):
        ...
        # If not, just move forward.
            ...
            # print("FSF")
            # print(self.isReward)
            # print(self.leftToReward)
            self.snake.move(self.isReward)
            if self.leftToReward > 0:
                # print("LEFT", self.leftToReward)
                # print(self.isReward)
                old_tail = None
                self.leftToReward -= 1
                if self.leftToReward == 0:
    ...
    def generate_good_fruit(self, position=None):
        """ Generate a new fruit at a random unoccupied cell. """
        if position is None:
            position = self.field.get_random_empty_cell()
        self.field[position] = CellType.GOOD_FRUIT
        self.goodFruit = position
